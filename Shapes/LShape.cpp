...
LShape::LShape(int colPair) : Shape(colPair) {
    ...
    //Shape's constructor will set a starting orientation, so just need
    //to compute the offsets
    computeNewOffsets();
}

LShape::LShape(int colPair, int orginY, int orginX) : Shape(colPair, orginY, orginX) {
    this->size = 4; //Apparently all tetris shapes are size 4
...
LShape::LShape(int colPair, int orginY, int orginX, int startingOrientation)
: Shape(colPair, orginY, orginX) {
    ...
    switch (startingOrientation % 4) {
        case POS0:
            break;
        case POS2: //so getting to POS2 takes 2 rotations, which is why there is no break
            rotateRight();
...
int LShape::getHeight() { //See header
...
int LShape::getWidth() { //See header
    if (POS0 == this->orientation || POS2 == this->orientation) {
        return 2;
    }
    else {
...
//Update: Handling rotations at the edge of the screen turned out to be trivial,
//        but I have the game working pretty well right now so no need to change
//        anything.
